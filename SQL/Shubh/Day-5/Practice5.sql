...
--2. names, department ID and department name of all the employees. 
SELECT CONCAT(E.FirstName,' ', E.LastName) AS 'Name', E.DepartmentID, D.DepartmentName 
	FROM Employees E JOIN Departments D ON E.DepartmentID = D.DepartmentID

--3. names, jobtitle, department id, and department name of the employees who work in Seattle. 
...
--4. employee id, name, their manager_id, manager name. 
SELECT CONCAT(E.FirstName,' ', E.LastName) AS 'EmployeeName', E.ManagerID, CONCAT(M.FirstName,' ', M.LastName) AS 'ManagerName' 
	FROM Employees E LEFT JOIN Employees M ON E.ManagerID = M.EmployeeID 

--5. department name, no. of employees in department. 
...
--6. all department ID, all department name and manager first name. 
SELECT D.DepartmentID, D.DepartmentName, CONCAT(M.FirstName,' ', M.LastName) AS 'ManagerName'  
	FROM Departments D LEFT JOIN Employees M ON D.ManagerID = M.EmployeeID
...
--8. job title, employee name, difference between salary of the employee and minimum salary for the job. 
SELECT E.JobId, CONCAT(E.FirstName,' ', E.LastName) AS 'Name', (E.Salary - M.Min_Salary) AS 'Difference_in_Salary' 
	FROM Employees E JOIN (SELECT JobId, MIN(Salary) AS 'Min_Salary' 
	FROM Employees GROUP BY JobId) M ON E.JobId = M.JobId

--9. employee history with more than 8000 salary. 
