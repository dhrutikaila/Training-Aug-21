...
SELECT FirstName+ ' ' +LastName AS Name FROM Employees WHERE DepartmentID IN (SELECT DepartmentID FROM Departments WHERE 
DepartmentName = 'IT' )
...
SELECT firstName+' '+LastName AS Name FROM Employees WHERE DepartmentID IN (SELECT DepartmentID FROM Departments 
WHERE LocationID IN (SELECT LocationID FROM Locations WHERE CountryID = 'US'))

--4. Find the names (first_name, last_name) of the employees who are managers.
SELECT FirstName+' '+LastName AS Name FROM Employees WHERE EmployeeID IN (SELECT EmployeeID FROM Employees WHERE EmployeeID
...
SELECT FirstName+' '+LastName AS Name,Salary FROM Employees WHERE Salary > (SELECT AVG(Salary) FROM Employees) AND 
...
SELECT FirstName+' '+LastName AS Name,Salary FROM Employees WHERE Salary > (SELECT MAX(Salary) FROM Employees WHERE 
JobId = 'SH_CLERK') ORDER BY Salary ASC

--12. Write a query to find the names (first_name, last_name) of the employees who are not supervisors. 
SELECT FirstName+' '+LastName AS Name from Employees WHERE JobId IN (SELECT JobId FROM Employees WHERE JobId != 'supervisors')

--13. Write a query to display the employee ID, first name, last names, and department names of all employees. 
SELECT E.EmployeeID,E.FirstName,E.LastName,D.DepartmentName FROM Employees E JOIN Departments D ON 
...
SELECT EmployeeID,FirstName LastName,Salary FROM Employees e1 WHERE Salary > 
...
SELECT * FROM (SELECT Salary,DENSE_RANK() OVER (ORDER BY Salary DESC) AS RANKNUMBER FROM Employees) 
AS TBL WHERE RANKNUMBER = 5

--17. Write a query to find the 4th minimum salary in the employees table. 
SELECT * FROM (SELECT DISTINCT Salary,DENSE_RANK() OVER (ORDER BY Salary ASC) AS RANKNUMBER FROM Employees) 
AS TBL WHERE RANKNUMBER = 4

--18. Write a query to select last 10 records from a table. 
SELECT * FROM (SELECT *,ROW_NUMBER() OVER (ORDER BY EmployeeID DESC) AS RANKNUMBER FROM Employees) AS 
TEMP WHERE RANKNUMBER <=10
...
SELECT * FROM (SELECT DISTINCT Salary , DENSE_RANK() OVER (ORDER BY Salary DESC) AS R FROM Employees)
AS TEMP WHERE R <4

--21. Write a query to get 3 minimum salaries. 
SELECT * FROM (SELECT DISTINCT Salary , DENSE_RANK() OVER (ORDER BY Salary ASC) AS R FROM Employees) 
AS TEMP WHERE R <=3

--22. Write a query to get nth max salaries of employees. 
DECLARE @VAR INT = 5;
SELECT DISTINCT Salary FROM (SELECT DENSE_RANK() OVER (ORDER BY Salary DESC) AS RANK,Salary FROM Employees) 
AS TEMP WHERE RANK = @VAR; 
