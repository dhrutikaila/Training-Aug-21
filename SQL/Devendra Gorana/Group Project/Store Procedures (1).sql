...
-- Store Procedure for insert or update new owner in Society_info and transfering data related to previous-owner's in member_info into Previous_Member_info.
	...
	BEGIN
		DECLARE @c int
		SET @c = 10		
		IF (SELECT TOP 1 MemberID FROM Member_info WHERE HouseNo=@c) >0
		BEGIN
			INSERT into Previous_Member_info SELECT * FROM Member_info WHERE HouseNo=@c;
			DELETE FROM Member_info WHERE HouseNo=@c;
		END
	...
	-- insert/update
	EXECUTE New_owner 10,'Rakesh Patel',8845246585,1
	GO
-----------------------------------------------------------------------------------------------------------------------------------------------
