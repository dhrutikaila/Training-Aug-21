...
SELECT DATEADD(DAY,1,EOMONTH(DATEADD(MONTH,-4,CAST(GETDATE() AS DATE)))) 'Date'

--2. Write a query to display the last day of the month (in datetime format) three months before the current month. 
SELECT EOMONTH(DATEADD(MONTH,-3,CAST(GETDATE() AS DATE))) 'Date'
...
SELECT EmployeeID
    , LastName
    , HireDate
    , DATEADD(MONTH,1,HireDate) 'Salary'
FROM Employees
