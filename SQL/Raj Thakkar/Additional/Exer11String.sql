...
SELECT JobId
     , STRING_AGG(CONVERT(VARCHAR,EmployeeID),', ')
FROM Employees
GROUP BY JobId
...
SELECT FirstName
     , PhoneNumber
     , REPLACE(PhoneNumber,'124','999') 'Updated'
FROM Employees
...
SELECT CONCAT('0', CONVERT(VARCHAR,MAX(Salary))) 'Max'
     , CONCAT('0', CONVERT(VARCHAR,MIN(Salary))) 'Min'
FROM Employees
...
SELECT StreetAddress
     , REVERSE(SUBSTRING(REVERSE(StreetAddress),1,CHARINDEX(' ',REVERSE(StreetAddress)))) 'Word'
FROM Locations
...
SELECT JobId
     , SUBSTRING(JobId,1,CHARINDEX('_',JobID)-1) 'Word'
FROM Employees
...
SELECT SUBSTRING(FirstName,1,8) AS 'Name'
     , ('$'+' '+Salary) AS Salary
FROM Employees E
