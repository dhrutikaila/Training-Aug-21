...
--Get department, total salary with respect to a department from employee table.
	SELECT D.DepartmentName,TBL.TotalSalary  FROM
	(SELECT DepartmentID,SUM(Salary) AS TotalSalary
	FROM Employees
	GROUP BY DepartmentID)TBL
	INNER JOIN Departments AS D ON D.DepartmentID = TBL.DepartmentID
...
SELECT * FROM 
	...
	FROM Employees
	GROUP BY DepartmentID)TBL
	ORDER BY TBL.[Maximum Salary] ASC
...
SELECT * FROM 
	...
	FROM Employees
	GROUP BY DepartmentID )TBL
	ORDER BY TBL.[Minimum Salary] ASC
...
SELECT * FROM 
	...
	HAVING SUM(Salary)>5000)TBL
	ORDER BY tbl.[Total Salary] DESC
...
--1.Get difference between JOINING_DATE and INCENTIVE_DATE from employee and incentives table
	...
	INNER JOIN Incentives AS I on E.EmployeeID=I.EmployeeID)TBL
...
--2.Select first_name, incentive amount from employee and incentives table for those employees who have incentives and incentive amount greater than 3000
	...
	where I.IncentiveAmount > 3000)TBL

--3.Select first_name, incentive amount from employee and incentives table for all employees even if they didn�t get incentives.
   	SELECT * FROM 
	(SELECT E.EmployeeID,E.FirstName,I.IncentiveAmount FROM Employees AS E
	LEFT JOIN Incentives AS I on E.EmployeeID=I.EmployeeId)TBL
	
--4.Select EmployeeName, ManagerName from the employee table.
   	SELECT * FROM 
	(SELECT E.FirstName AS EmployeeName,M.FirstName AS ManagerName
	FROM Employees AS E 
	LEFT JOIN Employees AS M ON E.ManagerID = M.EmployeeID)TBL
