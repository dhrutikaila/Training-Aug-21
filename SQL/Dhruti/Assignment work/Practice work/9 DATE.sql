...
--7.Write a query to get the current date in the following format. September 4, 2014
...
--12.Write a query to display the current date in the following format. Sample output : Thursday 4th September 2014
...
SELECT DATEPART(yyyy,HireDate) From Employee
GROUP BY DATEPART(yyyy,HireDate)
HAVING Count(EmployeeID)>10 


--17.Write a query to get the department ID, year, and number of employees joined. 
SELECT DepartmentID,'Year' = DATEPART(yyyy,HireDate),Count(EmployeeID)'count' From Employee
...
SELECT DISTINCT e.FirstName,e.LastName,DepartmentName,e.Salary FROM Employee e,Employee e1,Department d
...
SELECT EmployeeID,LastName,DATEADD(MM,1,Hiredate) SalaryDate from Employee

--20.Write a query to get first name, hire date and experience of the employees.
SELECT FirstName,HireDate,DATEDIFF(YYYY,HireDate,Getdate()) Experience FROM Employee
