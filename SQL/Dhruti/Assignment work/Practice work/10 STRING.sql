...
SELECT JobID,EmployeeID from Employee
...
LEFT(CONCAT(REPLICATE(0,10-LEN(MIN(salary))),MIN(SALARY)),10) from Employee
...
SELECT EmployeeID,FirstName,DATENAME(month,HireDate) [HireMonth] from Employee

--7. Write a query to get the employee id, email id (discard the last three characters). 
SELECT EMPLOYEEID,LEFT(EMAIL,LEN(EMAIL)-3) FROM Employee
...
SELECT RIGHT(PhoneNumber,4)  FROM Employee
...
SELECT SUBSTRING(REPLACE(JOBID,'_',' '),1,CHARINDEX(' ',REPLACE(JOBID,'_',' '))-1) FROM Employee
