...
SELECT * FROM Employees WHERE Salary > 8613
...
SELECT *
FROM(	SELECT ROW_NUMBER() OVER (ORDER BY EmployeeID) [Number] 
	,	*
	FROM Employees ) Temp
WHERE Temp.Number % 2 = 0


--16. Write a query to find the 5th maximum salary in the employees table. 

SELECT Salary 
FROM (SELECT DENSE_RANK() OVER(ORDER BY Salary DESC) [Rank_Sal], Salary
FROM Employees) Temp 
WHERE Rank_Sal = 5
...
FROM (SELECT DENSE_RANK() OVER(ORDER BY Salary ASC) [Rank_Sal], Salary
FROM Employees) Temp 
WHERE Rank_Sal < 4
