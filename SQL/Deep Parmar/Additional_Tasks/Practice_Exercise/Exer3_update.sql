...
--3) Write a SQL statement to change the Email and CommissionPct column of employees table with 'not available' 
...
--4) Write a SQL statement to change the Email column of employees table with 'not available' for those employees whose DepartmentID is 80
--and gets a commission is less than 20%

	 UPDATE Employees SET Email='not available',CommissionPct=0.10 WHERE DepartmentID=110 AND CommissionPct < 0.20
...
--5) Write a SQL statement to change the Email column of employees table with 'not available' for those employees who belongs to the 'Accounting' department.
	 ...
	 UPDATE Employees SET Email='not available' WHERE DepartmentID=(SELECT D.DepartmentID FROM Departments AS D WHERE DepartmentName='Accounting')
...
--7) Write a SQL statement to change job ID of employee which ID is 118, to SH_CLERK if the employee belongs to department, which ID is 30 
...
--and commission by 10%
 ...
 --UPDATE jobs SET MIN_SALARY=MIN_SALARY+2000,MAX_SALARY=MAX_SALARY+2000
	-- ,Salary=Salary+(Salary * 0.20),CommissionPct=CommissionPct+(CommissionPct * 0.10)
