...
SELECT FirstName,LENGTH(FirstName) FROM Employees WHERE (FirstName LIKE 'A%' OR FirstName LIKE 'J%' OR FirstName LIKE 'M%') ORDER BY FirstName; 
...
SELECT FirstName,RIGHT(replicate('$',10)+CAST(Salary AS VARCHAR(15)),10) AS SALARY FROM Employees;
...
SELECT EmployeeID as code,FirstName,LastName,HireDate FROM Employees where day(HireDate) = 7 or month(HireDate) = 7;
...
/*or*/
SELECT distinct FirstName,HireDate,datename(weekday,HireDate) as dayofwk FROM Employees where datename(weekday,HireDate) = 'Monday';
...
SELECT format(getdate(),'hh:mm tt')+' '+ datename(mm,getdate())+' ,'+cast(datepart(yyyy,getdate()) as varchar);


/* query11 */
/*Write a query to get the FirstName, LastName who joined in the month of June.*/

SELECT FirstName,LastName,HireDate FROM Employees where datename(mm,HireDate) = 'June';
